...
// Drag-to-reorder using the native HTML5 drag-and-drop API.
// Each draggable child must have [data-question-id]. A descendant with
...
export default class extends Controller {
  ...
  items() {
    return Array.from(this.element.children).filter((el) => el.dataset.questionId)
  }
  ...
  attachItem(item) {
    ...
    const onDragStart = (e) => {
      ...
      try { e.dataTransfer.setData("text/plain", item.dataset.questionId) } catch (_) { /* Safari */ }
    }
    const onDragEnd = () => {
  ...
  renumber() {
    this.element.querySelectorAll("[data-question-number]").forEach((el, idx) => {
      el.textContent = `${idx + 1}.`
    })
  }

  async persist() {
    const ids = this.items().map((el) => el.dataset.questionId)
